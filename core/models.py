...
from mysite.utils import valida_cedula, phone_regex
...
class Product(models.Model):
    ...
    price = models.DecimalField(
        "Precio", max_digits=10, decimal_places=2, default=Decimal("0.0"), validators=[MinValueValidator(Decimal('0.0'))]
    )
    ...
    iva = models.IntegerField(
        verbose_name="IVA", choices=((0, "0%"), (5, "5%"), (15, "15%")), default=15
    )
    ...
    supplier = models.OneToOneField(
        Supplier, on_delete=models.CASCADE, verbose_name="Proveedor"
    )
    categories = models.ManyToManyField("Category", verbose_name="Categoria")
    line = models.CharField(
        "Linea", max_length=2, choices=Status.choices, default=Status.COMISARIATO
    )
...
class Invoice(models.Model):
    ...
    discount = models.DecimalField(
        verbose_name="descuento", default=0, max_digits=16, decimal_places=2
    )
    ...
    status = models.CharField(
        verbose_name="Estado",
        max_length=1,
        choices=(("F", "Factura"), ("A", "Anulada"), ("D", "Devolucion")),
        default="F",
    )
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    created = models.DateTimeField(auto_now_add=True)
